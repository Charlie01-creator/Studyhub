...
export default async function HomePage() {
  // Everything that must succeed just to know "who is this and are they
  // suspended" goes through safeSupabase — if Supabase is unreachable or
  // misconfigured, we show a clear "temporarily unavailable" message
  // instead of crashing (this page has no auth middleware in front of it,
  // so it's the first thing a broken deployment would show).
  const initial = await safeSupabase(async (supabase) => {
  ...
  const { exam, user, isSuspended } = initial.data!;
  ...
  // a brand new project) doesn't blank the whole dashboard. Every branch
  // here has its own .catch() — previously the view-count query didn't,
  // which meant a transient failure there would reject the whole
  // Promise.all even though the other three had already caught their own
  // errors individually.
