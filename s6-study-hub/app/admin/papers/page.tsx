...
export default async function AdminPapersPage() {
  ...
  try {
    [papers, subjects] = await Promise.all([listPapers({}), listSubjects()]);
  } catch (err) {
    loadError = err instanceof Error ? err.message : "Failed to load papers.";
  }
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-6">
        <div role="alert" className="rounded-card bg-marker-red-soft px-4 py-3 text-sm text-marker-red">
          Couldn&apos;t load the admin dashboard right now: {loadError}
        </div>
      </div>
    );
  }
